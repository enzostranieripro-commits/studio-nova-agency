...
const cities = [
  ...
  'Najac', 'Sévérac-d\'Aveyron', 'Saint-Geniez-d\'Olt', 'Marcillac-Vallon',
  // Occitanie
  'Toulouse', 'Montpellier', 'Nîmes', 'Albi', 'Cahors', 'Montauban', 'Figeac',
  'Mende', 'Aurillac', 'Florac',
  // France entière
  'Paris', 'Lyon', 'Bordeaux', 'Marseille', 'Nantes', 'Lille', 'Strasbourg',
];
      ...
      <div className="animate-marquee-slow flex whitespace-nowrap">
        {items.map((city, i) => (
          <span key={i} className="flex items-center gap-2 mx-4">
            <MapPin size={10} className="text-primary/40 flex-shrink-0" />
            <span className="text-[0.65rem] uppercase tracking-[0.2em] text-muted-foreground/50 font-medium">{city}</span>
          </span>
        ))}
      </div>
    </div>
