...
import { Globe, Layers, ShoppingCart, Check, ChevronRight, ChevronLeft, Zap, Star, Users } from 'lucide-react';
...
const offerData = [
  {
    ...
    desc: 'Une page unique, percutante, optimisée pour convertir vos visiteurs en clients.',
  ...
  },
  {
    ...
    desc: 'Un site professionnel complet qui inspire confiance et génère des contacts qualifiés.',
  ...
  },
  {
    ...
    desc: 'Vendez en ligne avec un système complet et autonome, prêt à générer du chiffre.',
...
const colorMap: Record<string, { text: string; bg: string; border: string; accent: string; pill: string }> = {
  visibility: {
    text: 'text-visibility',
    bg: 'bg-visibility/10',
    border: 'border-visibility/25',
    accent: 'hsl(var(--offer-visibility))',
    pill: 'bg-visibility/15 text-visibility border border-visibility/20',
  },
  authority: {
    text: 'text-authority',
    bg: 'bg-authority/10',
    border: 'border-authority/25',
    accent: 'hsl(var(--offer-authority))',
    pill: 'bg-authority/15 text-authority border border-authority/20',
  },
  conversion: {
    text: 'text-conversion',
    bg: 'bg-conversion/10',
    border: 'border-conversion/25',
    accent: 'hsl(var(--offer-conversion))',
    pill: 'bg-conversion/15 text-conversion border border-conversion/20',
  },
};
              ...
              {offerData.map((o, i) => {
                ...
                return (
                  <motion.button
                    ...
                    className={`bg-card rounded-2xl border p-6 text-left transition-all relative overflow-hidden group ${o.badge ? `border-2 ${oc.border}` : 'hover:border-foreground/15'}`}
                  >
                    {/* Color bar top */}
                    <div className="absolute top-0 left-0 right-0 h-0.5 opacity-70" style={{ background: oc.accent }} />

                    {/* Popular badge */}
                    {o.badge && (
                      <div className="absolute -top-px left-1/2 -translate-x-1/2">
                        <span className="text-[0.6rem] uppercase tracking-widest font-semibold bg-authority text-white px-4 py-1 rounded-b-lg shadow-lg block">
                          {o.badge}
                        </span>
                      </div>
                    )}

                    <div className={`w-10 h-10 rounded-xl ${oc.bg} flex items-center justify-center mb-4 mt-3`}>
                      <o.icon className={oc.text} size={18} />
                    </div>

                    {/* Name & tagline */}
                    <h3 className={`font-display text-xl font-bold mb-0.5 ${oc.text}`}>{o.name}</h3>
                    <p className="text-xs text-muted-foreground mb-3 font-medium">{o.tagline}</p>
                    <p className="text-xs text-muted-foreground mb-4 leading-relaxed">{o.desc}</p>

                    {/* Price block */}
                    <div className="mb-4 p-3 rounded-xl bg-foreground/3 border border-foreground/5">
                      <p className="text-[0.65rem] text-muted-foreground uppercase tracking-widest mb-1">à partir de</p>
                      <div className="flex items-baseline gap-1">
                        <span className={`font-display text-3xl font-bold ${oc.text}`}>{o.price.monthly}€</span>
                    ...
                    <div className="flex items-center gap-2 mb-4 text-xs text-muted-foreground">
                      <Users size={11} className={oc.text} />
                      <span><strong className="text-foreground">{o.users}</strong> clients sur cette offre</span>
                    </div>

                    <div className={`flex items-center justify-center gap-1 text-xs font-semibold py-2 rounded-xl ${oc.bg} ${oc.text} border ${oc.border}`}>
                      Choisir cette offre <ChevronRight size={13} />
                    </div>
                  </motion.button>
                );
              })}
          ...
          {step === 1 && offer && c && (
            ...
            >
              <div className="bg-card rounded-2xl border p-7">
                ...
                {/* Audit booked count */}
                <div className="flex items-center gap-2 p-3 rounded-xl bg-foreground/3 border border-foreground/6 mb-6 text-xs text-muted-foreground">
                  <Star size={11} className={c.text} />
                  <span><strong className="text-foreground">{offer.users} clients</strong> ont déjà réservé leur premier audit sur cette offre</span>
                </div>
          ...
          {step === 3 && offer && c && (
            ...
            >
              ...
              <div className="bg-card rounded-2xl border p-6 mb-5">
                ...
                <div className="space-y-2 text-sm mb-4">
                  <div className="flex justify-between items-center">
                    ...
                    <span className={`font-display font-bold text-lg ${c.text}`}>
                      ...
                      {selectedFormat !== 'once' ? <span className="text-xs text-muted-foreground font-normal">/mois</span> : ''}
                    </span>
                  </div>
                  {offer.options.map((opt, i) =>
                    selectedOptions[i] ? (
                ...
                <div className="border-t border-foreground/8 pt-4 flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <span className={`font-display text-4xl font-bold ${c.text}`}>
                      {getTotal()}€
                    </span>
                    {selectedFormat !== 'once' && (
              ...
              <div className="flex items-center gap-2 mb-5 text-xs text-muted-foreground justify-center">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} size={12} className="text-conversion fill-conversion" />
                ))}
